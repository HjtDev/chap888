...
class Order(models.Model):
    ...
    email = models.EmailField(
        max_length=255, verbose_name="ایمیل", blank=True, null=True
    )
    phone = models.CharField(max_length=11, verbose_name="شماره تلفن")
    province = models.CharField(
        max_length=255, verbose_name="استان", blank=True, null=True
    )
    city = models.CharField(
        max_length=255, verbose_name="شهرستان", blank=True, null=True
    )
    address = models.CharField(
        max_length=300, verbose_name="آدرس", blank=True, null=True
    )
    postal_code = models.CharField(
        max_length=10, verbose_name="کد پستی", blank=True, null=True
    )
    notes = models.TextField(
        max_length=500, verbose_name="یادداشت های سفارش", blank=True, null=True
    )
    ...
    class OrderStatus(models.TextChoices):
        ...
        READY_TO_DELIVER = "آماده تحویل", _("آماده تحویل")
    ...
    def get_total_price(self, post_price, discount: "Discount" = None) -> (int, int):
        ...
        return price, (
            (price + post_price - discount.calculate(price))
            if discount
            else price + post_price
        )
...
class OrderItem(models.Model):
    ...
    size = models.CharField(
        max_length=2, choices=SizeChoices.choices, verbose_name="اندازه"
    )
    color = models.CharField(
        max_length=21, choices=ColorChoices.choices, verbose_name="رنگ"
    )
    print_type = models.CharField(
        max_length=21, choices=TypeChoices.choices, verbose_name="نوع چاپ"
    )
    extra = models.CharField(
        max_length=21, choices=ExtraChoices.choices, verbose_name="نوع صحافی"
    )
    ...
    def get_item_cost(self):
        ...
        return (
            (
                prices.get(self.size)
                + prices.get(self.color)
                + prices.get(self.print_type)
                + prices.get(self.extra)
            )
            * pages
            * self.quantity
        )
...
class Transaction(models.Model):
    ...
    reason = models.CharField(
        verbose_name="علت تراکنش", choices=ReasonChoice.choices, max_length=21
    )
    status = models.CharField(
    ...
    if "%" in value:
        ...
        if not value.isdigit() or not (0 < int(value) <= 1_000_000):
            raise ValidationError(
                "مقدار تخفیف به تومان باید فقط یک عدد بین 1 تا 1،000،000 باشد."
            )
...
class Discount(models.Model):
    ...
    value = models.CharField(
        ...
        help_text="تخفیف به تومان یا درصد",
        validators=[discount_value_validator],
    )
    expire_at = jmodels.jDateTimeField(
        verbose_name="تاریخ انقضا", default=datetime.today
    )

    used_by = models.ManyToManyField(
        User, related_name="used_discounts", verbose_name="کاربر"
    )
    ...
    def validate(self, phone):
        return not (
            datetime.now() > self.expire_at or self.used_by.filter(phone=phone).exists()
        )
